...
function[S1] = FiltroECGMuscular_WaveletFs256_TesteArduino_v3(sinal,sfreq,name_filtro)
...
%% Limiarização do coeficiente cD3 %%
...
while jj <= length(cD3h)-swin
    avgs = mean(cD3h(jj:jj+swin-1));
...
%     vthr(ii) = max(abs(cD3(vend(ii):vbeg(ii+1))));
% end
% v4max = zeros(1,5);


for ii = 1:length(vend) - 1
    cD3(vend(ii):vbeg(ii+1))=0;
end
...
% figure;plot(cD3);
% hold on;plot(lim_def*ones(1,length(cD3)),'k');
% plot(-lim_def*ones(1,length(cD3)),'k');

% keyboard;
% posz = find(abs(cD3)<=lim_def);
% cD3(posz)=0;
% posnz1 = find(cD3>lim_def);
% cD3(posnz1) = cD3(posnz1) - lim_def;
% posnz2 = find(cD3<-1*lim_def);
% cD3(posnz2) = cD3(posnz2) + lim_def;
% 
% keyboard;
