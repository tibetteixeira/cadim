...
function[S1] = FiltroECGMuscular_WaveletFs256_TesteArduino_v4(sinal,sfreq,name_filtro)
...
%% Selecionando limiar para seleção dos coeficientes cD3 %%%

hcD3 = (median(abs(cD3-median(cD3)))/(0.6745))*(4*length(cD3)/3)^(-0.2);

mincD3 = min(cD3);
maxcD3 = max(cD3);
xcD3 = linspace(mincD3,maxcD3,100);
%xcD3 = mincD3:hcD3:maxcD3;
VetorF = DensEst(xcD3,cD3,hcD3);

dx = xcD3(2)-xcD3(1);
cF = [];
cF(1) = 0;
for ii = 2:length(VetorF)
    cF(ii) = sum(VetorF(1:ii)*dx);
end

ixalvo = find(cF > 0.1*max(cF) & cF < 0.9*max(cF));
lth = xcD3(ixalvo(1));
uth = xcD3(ixalvo(end));
